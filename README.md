...
```mermaid
flowchart TD
    Start(["🤔 我现在该用哪个命令?"]) --> Q1{"这个项目<br/>套过救命套件了吗?"}
    Q1 -->|"❌ 还没套 (新项目第一次)"| Setup["🆕 /setup-kit<br/>装 7 件套 + 专属 onboard/offboard<br/>(每个项目一辈子只做这一次)"]
    Q1 -->|"✅ 已经套过了"| Q2{"你现在是<br/>开工 还是 收工?"}
    Q2 -->|"🌅 刚开新窗口, 要接着做"| On["🪄 /缩写-onboard<br/>Claude 读手册 → 汇报状态 → 等你指示"]
    Q2 -->|"🌙 干完了 / 窗口快满 / 下班"| Off["🛑 /缩写-offboard<br/>9 步存档: 更新进度 + commit + 留便条"]
    Setup -.->|"套完之后<br/>每天开窗都走这条"| On
    On --> Work["💪 你指示, Claude 干活"]
    Work --> Off
    Off -.->|"明天 / 下周再开窗"| On
```

**一句话**：新项目用**一次** `/setup-kit`；之后**每天开窗 `onboard`、收工 `offboard`**，循环往复。
...
| 场景 | 痛点 | 👉 对应方案 |
|------|------|-----------|
| 🕳️ 早上开新窗口 | Claude 从 0 开始摸索，浪费 30 分钟上下文 | [ONBOARDING.md 黄金标准](docs/2-七件套骨架详解.md) + [ADR 决策记录](docs/7-ADR-架构决策记录.md) |
| 💀 电脑硬盘坏了 | 配置密钥全丢，项目无法恢复 | [备份分层策略](docs/4-备份分层.md) |
| 🪤 以为 Phase 2 完成了 | 其实代码根本没 commit，重启会话一查发现什么都没有 | [Git 起步 6 步](docs/5-git-起步-6-步.md) |
| 🎭 多项目名字相似 | "奢侈品库存 App" vs "AI 买手" 搞混，改错项目 | [Obsidian 文档规范：命名](docs/10-Obsidian-文档规范.md) |
| 🔑 密钥被 Claude 推到 GitHub | 后悔莫及 | [密钥管理三位一体](docs/3-密钥管理三位一体.md) + [pre-commit hooks](docs/9-pre-commit-hooks.md) |
| 🔄 每次部署靠手敲命令 | 一紧张少打一个字，服务挂掉 | [deploy.sh 一键部署](docs/2-七件套骨架详解.md#scriptsdeploysh-一键部署脚本) |
